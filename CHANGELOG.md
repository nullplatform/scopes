...
- Fix: a namespace containing shell metacharacters is no longer re-parsed by the shell when fetching logs. The channel that was exploitable is the `NAMESPACE_OVERRIDE` environment variable on the agent, which the log reader expanded a second time; the scope's cluster configuration only becomes a namespace source in this same change, and is inert from the start
