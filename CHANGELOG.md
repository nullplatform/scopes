...
## [Unreleased]
- Application logs can now be routed to CloudWatch or Datadog through the new `logging.provider` scope configuration (or the `LOGS_PROVIDER` env var), set organization-wide or per environment. A scope may override it per-scope with the `logs_provider_override` capability, now declared by the k8s service specification and surfaced in the UI as **Logs provider**. Only application logs are affected; CloudWatch metrics, access logs and the nullplatform log viewer are unchanged
- The annotation that gates the logs is now configurable through the new `logging.annotation_prefix` scope configuration (or the `LOGS_ANNOTATION_PREFIX` env var), for clusters that already carry `nullplatform.logs.cloudwatch: 'true'` on every deployment and need to opt a subset in rather than re-annotate everything. It must match `CLOUDWATCH_LOGS_ANNOTATION` / `DATADOG_LOGS_ANNOTATION` on that cluster's logs controller, which requires logs-controller 1.6.0 or newer. While the prefix is not the default, pods carry both the prefixed and the default key so the two sides can be rolled out in either order
- Defaults are unchanged end to end: an account that configures neither setting renders exactly the annotations it rendered before, so no controller upgrade is needed to take this release
