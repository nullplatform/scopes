...
@test "build_blue_deployment: discovers the blue CronJob's real name after a strategy change" {
  local raw_context
  raw_context="$(cat "$PROJECT_ROOT/k8s/naming/tests/fixtures/context-normal.json")"

  export NAMING_STRATEGY="qualified"

  # The blue was created back when the strategy was "ids"; its live CronJob is
  # still named job-123456-789011, and carries no trace of "qualified".
  kubectl() {
    case "$1 $2" in
      "get deployment") echo '{"items":[]}' ;;
      "get service")    echo '{"items":[]}' ;;
      "get cronjob")    echo "job-123456-789011" ;;
      *)                echo "" ;;
    esac
  }
  export -f kubectl
