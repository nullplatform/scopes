...
#      Created from ./hashicorp-vault-configuration.json.tpl. The JSON file is
...
#      dimensions, and Vault address — so operators can point different
...
resource "nullplatform_api_key" "this" {
  ...
  name = "hashicorp-vault-api-key-${each.key}"
  dynamic "grants" {
