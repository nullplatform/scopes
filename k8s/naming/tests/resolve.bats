...
setup() {
	...
	export -f get_config_value np_naming_strategy np_naming_resolve \
		np_naming_roles_ids np_naming_roles_patterned np_naming_emit
...
@test "np_naming_resolve: ids reproduces today's dns-endpoint truncation" {
	run bash -c "np_naming_resolve | jq -r .scope_dns"
...
@test "np_naming_resolve: ids keeps the serving-cert name separate from the ingress base" {
	run bash -c "np_naming_resolve | jq -r '.serving_cert, .scope_ingress'"
...
@test "np_naming_resolve: leaves deployment fields empty without a deployment id" {
	...
	run bash -c "np_naming_resolve | jq -r '.deployment, .hpa, .secret'"
...
@test "np_naming_resolve: enriches additional ports with service and ingress names" {
	run bash -c "np_naming_resolve | jq -r '.additional_ports[0].service_name, .additional_ports[0].ingress_name'"
