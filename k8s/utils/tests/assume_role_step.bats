...
setup() {
  ...
  # CONTEXT with the IAM provider (resolved) carrying a containers role.
  export CONTEXT='{"providers":{"identity-access-control":{"iam_role_arns":{"arns":[{"selector":"containers","arn":"arn:aws:iam::111:role/containers-role"}]}}}}'
  unset CONTAINERS_ASSUME_ROLE_ARN CONTAINERS_ASSUME_ROLE_ARN_DEFAULT CONTAINERS_ASSUME_ROLE_SELECTOR \
...
@test "assume_role_step: resolves the containers role from CONTEXT and exports creds" {
  logf=$(mktemp)
  source "$STEP" >"$logf" 2>&1
  [ "$CONTAINERS_ASSUME_ROLE_ARN" = "arn:aws:iam::111:role/containers-role" ]
...
@test "assume_role_step: uses the dimension-resolved provider already in CONTEXT" {
  ...
  export CONTEXT='{"providers":{"identity-access-control":{"iam_role_arns":{"arns":[{"selector":"containers","arn":"arn:aws:iam::111:role/us-east-1-role"}]}}}}'
  logf=$(mktemp)
  source "$STEP" >"$logf" 2>&1
  [ "$CONTAINERS_ASSUME_ROLE_ARN" = "arn:aws:iam::111:role/us-east-1-role" ]
  assert_contains "$(cat "$logf")" "   🔑 Assuming role: arn:aws:iam::111:role/us-east-1-role"
  assert_contains "$(cat "$logf")" "   ✅ Role assumed successfully"
}

@test "assume_role_step: honors CONTAINERS_ASSUME_ROLE_SELECTOR override" {
  export CONTAINERS_ASSUME_ROLE_SELECTOR="custom"
  export CONTEXT='{"providers":{"identity-access-control":{"iam_role_arns":{"arns":[{"selector":"custom","arn":"arn:aws:iam::111:role/custom-role"}]}}}}'
  logf=$(mktemp)
  source "$STEP" >"$logf" 2>&1
  [ "$CONTAINERS_ASSUME_ROLE_ARN" = "arn:aws:iam::111:role/custom-role" ]
}

@test "assume_role_step: pre-set CONTAINERS_ASSUME_ROLE_ARN overrides provider resolution" {
  export CONTAINERS_ASSUME_ROLE_ARN="arn:aws:iam::111:role/explicit-override"
  logf=$(mktemp)
  source "$STEP" >"$logf" 2>&1
  [ "$CONTAINERS_ASSUME_ROLE_ARN" = "arn:aws:iam::111:role/explicit-override" ]
  [ "$AWS_ACCESS_KEY_ID" = "AKIA1" ]
}
...
@test "assume_role_step: exits non-zero with full hints when sts:AssumeRole fails" {
  ...
  [ "$status" -ne 0 ]
  assert_contains "$output" "   ❌ assume_role step failed: could not assume arn:aws:iam::111:role/containers-role"
